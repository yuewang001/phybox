...
{
	printf("__rx_event socket thread begin：\n");
	while(1)
	{
	    //printf("__rx_event socket thread begin：\n");
	    sleep(5);
...
{
	printf("thread __gpsTimer_event begin:\n");
...
{
	printf("function  PID_Sync_WriteConfig starts:\n");
	return NoError;
	struct dev_buff *dev;
	struct enet_frame *frame;
	struct ctl_pkt	*pkt;
	...
	*/
	...
	printf("function  PID_Sync_WriteConfig end:\n");
...
{
	printf("rt_ioctl function begin,input request:%d\n",request);

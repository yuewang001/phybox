 ...
 */
pthread_t pid_o;
static unsigned int pthread_o = 0;
void *create_rt_thread_(void * func)
{
	//dpdk_start_launch_on_core(func, NULL, rt_core_id);
	printf("create_rt_thread_: to create a rt thread!\n");

		int core;
		core = allocate_rtcore();

		printf("pthread_create(create_rt_thread_): allocate_rtcore,core = %d\n",core);
		if (core < 0)
		{
			printf("PHY pthread_create(create_rt_thread_): allocate_rtcore failed,core = %d\n",core);
			return -1;
		}
		else
		...
		pthread_create(&pid_o, NULL, func, &pthread_o);
		printf("new created thread num:%d\n",pid_o);

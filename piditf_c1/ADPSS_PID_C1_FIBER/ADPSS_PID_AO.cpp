...
{
	I32 i, n=size();
	U16 slot_info[MAX_SLOTS][7], slot_flag[MAX_SLOTS];
	...
	if (n>0)
	{
		...
		{
			ret_test =  PID_Slot_ReadInfo(DeviceId, i, &slot_info[i][0], &All_Slp[i][0], &All_Itcp[i][0]);
		//	printf("ret_test =%d in AO \n",ret_test);
		}
		...
		for (i=0; i<n; i++)
		{
			...
			if (slot_info[var.slotno][1] == CARD_INFO_AO)			// 该信号对应槽位的板卡为AO卡
			{
	...
	//	*(U32 *)&config[1] = 50;
		...
		*(U32 *)&config[1] =(U32)(syncDT*1000000);
		...
		for (i=0; i<MAX_SLOTS; i++)
		{
			...
			{	
				config[0] = CARD_CFG_AO_SIM;
				PID_Slot_WriteConfig(DeviceId, i, config);
			}
		}

		//  0 - AO Parameter Set or Check
		//  Channel        23        22        21        20        19        18
		//     K      3276.80   3276.80   3276.80   3276.80   3276.80   3276.80
		//     LP     -10.000   -10.000   -10.000   -10.000   -10.000   -10.000
		...
		if(err != 0)
		{
			fprintf(fdbg, "CADPSS_PID_AO::init(), PIDNo=%d, Slp Err Itcp Err return -3\n", id);

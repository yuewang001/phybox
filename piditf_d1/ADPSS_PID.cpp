...
CADPSS_PID::CADPSS_PID(U16 nPIDNo){
	...
	PIDNo=nPIDNo;
	fprintf(fdbg,"CADPSS_PID::CADPSS_PID: PIDNo:%d\n",PIDNo);
	isMaster=FALSE;
}
...
I32 CADPSS_PID::init(F64 SyncDT)
{
	...
	//printf("CADPSS_PID::init suppose PIDNo=0\n");
	//PIDNo=0;


#if PID_SEM_MODE
	fprintf(fdbg,"CADPSS_PID PIDNo=%d  init: MODE_SEM\n",PIDNo);
	PID_Register(PIDNo, MODE_SEM);				//初始化
#else
	fprintf(fdbg,"CADPSS_PID  PIDNo=%d init: MODE_QRY\n",PIDNo);
...
//	printf("ret_test =%d syncDT=%f syncFreq=%d \n",ret_test,syncDT,syncFreq);
	...
	//板卡初始化
	nret=-1;
	fprintf(fdbg,"CADPSS_PID::init, PIDNO:%d\n",PIDNo);
...
I32 CADPSS_PID::SenddataToPID(U16 DeviceId)
{
	...
	{
		if(DEBUG)
		fprintf(fdbg,"vao SendDataToPID\n");
	...
	{
		if(DEBUG)
		fprintf(fdbg,"vdo SendDataToPID\n");

		nret+=vdo.SenddataToPID();
	}

	if(DEBUG)
	fprintf(fdbg,"CADPSS_PID::SenddataToPID: to call PID_Update_NewData,DeviceId:%d \n",DeviceId);

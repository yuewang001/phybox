							...
							I32 VarNo, U16 nPIDNo, U16 SlotNo, U16 nChanNo, F64 KAmplify)
{
	I32 i;
	if(DEBUG)
	fprintf(fdbg,"CADPSS_PID_ITF::addavar input idxio=%d,idno=%d,"
			"signType=%d,nPIDNo=%d, \n",idxio,idno,signType,nPIDNo);
	//printf("CADPSS_PID_ITF::addavar only for debug ,and set nPIDNo=0\n");

	/*
	if(DEBUG)
	{
	   nPIDNo=0; //only for debug by Wang Yue
	}
	*/
...
I32 CADPSS_PID_ITF::SenddataToPID()
{
	...
	for (i=0; i<MAX_PID_NUM_PER_PROC; i++)
	{
		if (pid[i])
		{
			if(DEBUG)
			{
			   fprintf(fdbg," CADPSS_PID_ITF::SenddataToPID: to call SendataToPID(%d)\n",i);
			}
...
I16 CADPSS_PID_ITF::MarkInput_Wait(U16 Timeout)
{
	...
	if(DEBUG)
	{
	   fprintf(fdbg,"CADPSS_PID_ITF::MarkInput_Wait:sem mode---------------\n");
	}
	for (i=0; i<MAX_PID_NUM_PER_PROC; i++)
    {
		if (pid[i])	{         
	...
	if(DEBUG)
	{
	    fprintf(fdbg,"CADPSS_PID_ITF::MarkInput_Wait:updates mode---------------\n");
	}
	for (i=0; i<MAX_PID_NUM_PER_PROC; i++)
	{
		if (pid[i])	{  

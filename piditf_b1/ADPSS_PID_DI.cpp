                     ...
                     I32 VarNo, U16 SlotNo, U16 nChanNo, F64 KAmplify)
{
   ...
   fprintf(fdbg, "CADPSS_PID_DI::idxio=%d, varno=%d, slotno=%d, ch=%d, valuetype=%d\n", idxio, VarNo, SlotNo, nChanNo, ValueType);
...
I32 CADPSS_PID_DI::init(U16 id)
{
   ...
   if (n>0)
   {
      ...
      for (i=0; i<n; i++)
      {
         ...
         if (slot_info[var.slotno] == CARD_INFO_DI)         // 该信号对应槽位的板卡为DI卡
         {
      ...
      for (i=0; i<MAX_SLOTS; i++)
      {
         if (slot_flag[i] == 1)     // 该槽位为DI卡，且有通道被调用
            PID_Slot_WriteConfig(DeviceId, i, 0x0f00 | CARD_CFG_DI_SIM_ELEC);
      }
